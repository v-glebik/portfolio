...
import { fetchUserAction } from '../../src/services/users/actions';
...
it('Dispatches the correct fetchUserAction and payload', () => (
  store.dispatch(fetchUserAction()).then(() => (
...
it('error fetch user list', () => {
  const expectedActions = [
    {
      type: types.FETCH_USER_WAITING
    },
    {
      type: types.UNAUTHENTICATION,
      payload: 'Request failed with status code 401'
    }
  ];
  return store.dispatch(fetchUserAction()).then(() => {
...
it('correct fetch user list', async () => {
  ...
  // check user list
  return store.dispatch(fetchUserAction(1, 1)).then(() => {
    expect(store.getActions()).toMatchObject([
      { type: 'FETCH_USER_WAITING' },
